...
class listingModel extends Model {
	...
	public function listFellows($type,$select) {
		...
		if($type == 'women') {
			...
			$sth = $dbh->prepare('SELECT * FROM ' . FELLOW_TABLE . ' WHERE sex=\'F\' AND type=\'\' ORDER BY name');
		}
		elseif($type == 'deceased,women') {

			$sth = $dbh->prepare('SELECT * FROM ' . FELLOW_TABLE . ' WHERE sex=\'F\' AND type REGEXP \'deceased\' ORDER BY name');
		}
		else{

			$sth = $dbh->prepare('SELECT * FROM ' . FELLOW_TABLE . ' WHERE type=:type AND name REGEXP :select ORDER BY name');
		}
	...
	public function listCoverPages($journal = DEFAULT_SERIES) {
		...
		$path = PHY_VOL_URL . $journal . '/**/**/cover.jpg';
		$files = glob($path);
		$covers = array();
