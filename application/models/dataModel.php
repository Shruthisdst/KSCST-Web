...
class dataModel extends Model {
	...
	public function getFellowDetailsfromCSV() {
		...
		$fileName = XML_SRC_URL . 'present-fellows.csv';

		if (!(file_exists(PHY_XML_SRC_URL . 'present-fellows.csv'))) {
		...
		foreach ($fellows as $fellowCSV) {
			...
			$fellow['address'] = $details[6] . ' ' . $details[7] . ' ' . $details[8] . ' ' . $details[9];
			$fellow['birth'] = $details[2];
			...
			$fellow['fname'] = $details[0];
			$fellow['lname'] = $details[1];
			$fellow['section'] = $details[19];
			$fellow['sex'] = $details[23];
		...
		$fileName = XML_SRC_URL . 'deceased-fellows.csv';

		if (!(file_exists(PHY_XML_SRC_URL . 'deceased-fellows.csv'))) {
		...
		foreach ($fellows as $fellowCSV) {
			...
			$fellow['birth'] = $details[2];
			$fellow['city'] = $details[10];
			$fellow['councilservice'] = $details[15];
			$fellow['country'] = $details[11];
			$fellow['death'] = $details[16];
			$fellow['degree'] = $details[4];
			$fellow['email'] = '';
			$fellow['fax'] = '';
			$fellow['honours'] = $details[5];
			...
			$fellow['fname'] = $details[0];
			$fellow['lname'] = $details[1];
			$fellow['section'] = $details[13];
			$fellow['sex'] = $details[17];
			$fellow['specialization'] = $details[12];
			$fellow['state'] = '';
			$fellow['telephone_mobile'] = '';
			$fellow['telephone_office'] = '';
			$fellow['telephone_residence'] = '';
			$fellow['type'] = 'deceased';
			$fellow['url'] = '';
			$fellow['yearelected'] = $details[14];
			$fellow['id'] = str_pad($id, 5, '0', STR_PAD_LEFT);
			$id++;
